...
_SKILL_REVIEW_PROMPT = (
    ...
    "Namespace decomposition (critical): if your concept decomposes "
    "into a real hierarchy, encode the namespace in the `category:` "
    "field as a `/`-separated path and keep `name:` as the leaf. The "
    "shape is `<domain>/<broader-category>/<specific-topic>/SKILL.md` "
    "with the broadest concept at the top and the most specific at the "
    "bottom. Example: a skill about reload-verification work that "
    "lives in the `hermes` project, inside the `plugins` subsystem, "
    "is encoded as `category: hermes/plugins` and `name: "
    "reload-verification` — NOT as a flat `name: hermes-plugin-reload"
    "-verification` under a single-segment `category: devops`. Do "
    "NOT flatten a real namespace hierarchy into a single hyphen-"
    "joined name. `_validate_name` rejects names whose shape suggests "
    "stuffed namespace components when no decomposition is provided, "
    "but the right outcome is the multi-segment encoding, not the "
    "stuffed flat name. Top-level domains are singular (`hermes/`, "
    "`github/`, `linux/`); sub-categories that group a collection of "
    "skills are plural (`hermes/plugins/`, `hermes/diagnostics/`, "
    "`linux/services/`); discipline or activity names stay singular "
    "(`hermes/diagnostics/reasoning-leak` — `diagnostics` is a "
    "discipline, not a collection of skills). Before creating, run "
    "`skill_view` on any existing umbrella that overlaps the "
    "namespace you intend to use; if one already exists, prefer "
    "(2) UPDATE AN EXISTING UMBRELLA over (4). Sub-categories must "
    "name a more specific concept than their parent, not a synonym of "
    "it: `software-development/development/...` is a tautology "
    "(`development` restates the parent), and so is "
    "`hermes/agent/agent/...`. A sub-category should name a distinct "
    "discipline, tool, phase, named subsystem, or axis from its "
    "parent. If the sub-category would just restate the parent, drop "
    "it and put the leaf directly under the parent.\n\n"
