...
class TestFetchOpenRouterModels:
    def test_live_fetch_recomputes_free_tags(self, monkeypatch):
        ...
        # Pin the curated-manifest accessor out so the preferred id list comes
        # from the in-repo OPENROUTER_MODELS snapshot, not a live network fetch
        # of the remote catalog (unreachable/frozen in CI would otherwise make
        # this assertion depend on the deployed manifest's contents).
    ...
    def test_permissive_when_supported_parameters_missing(self, monkeypatch):
        ...
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        # Hermetic: pin the curated-manifest accessor so the preferred id list
        # is the in-repo snapshot rather than a live remote-catalog fetch.
        with patch("hermes_cli.model_catalog.get_curated_openrouter_models", return_value=None), \
             patch("hermes_cli.models._urlopen_model_catalog_request", return_value=_Resp()):
            models = fetch_openrouter_models(force_refresh=True)
